(() => {
    ...
    (() => {
        ...
        function rangeInit() {
            ...
            if (priceSlider) {
                let textFrom = priceSlider.querySelector("[data-from]");
                let textTo = priceSlider.querySelector("[data-to]");
                let text = priceSlider.querySelector("[data-item]");
                nouislider.create(text, {
                    start: [ Number(textFrom.value), Number(textTo.value) ],
                    tooltips: [ true, true ],
                    connect: true,
                    range: {
                        min: [ Number(textFrom.value) ],
                        max: [ Number(textTo.value) ]
                    }
                });
                text.noUiSlider.on("update", (function(values, handle) {
                    textFrom.value = values[handle];
                    textTo.value = values[handle];
                }));
            }
            const priceSlider2 = document.querySelector("[data-range-range]");
            if (priceSlider2) {
                let textFrom2 = priceSlider2.querySelector("[data-range-from]");
                let textTo2 = priceSlider2.querySelector("[data-range-to]");
                let text2 = priceSlider2.querySelector("[data-range-item]");
                nouislider.create(text2, {
                    start: [ Number(textFrom2.value), Number(textTo2.value) ],
                    tooltips: [ true, true ],
                    connect: true,
                    range: {
                        min: [ Number(textFrom2.value) ],
                        max: [ Number(textTo2.value) ]
                    }
                });
                text2.noUiSlider.on("update", (function(values, handle) {
                    textFrom2.value = values[handle];
                    textTo2.value = values[handle];
                }));
            }
        }
        rangeInit();
            ...
            if (document.querySelector(".main-block__slider")) new core(".main-block__slider", {
                ...
                on: {
                    ...
                    slideChange: function(swiper) {
                        ...
                        currentSlide.innerHTML = swiper.activeIndex + 1 < 10 ? `0${swiper.activeIndex + 1}` : swiper.activeIndex.lenght;
                        console.log(swiper);
                    }
                }
            });
        ...
        if (MenuBlocks.lenght) MenuBlocks.forEach((menuBlock => {
            const menuBlockItems = menuBlock.querySelectorAll(".sub-menu-catalog__category").lenght;
        ...
        function documentActions(e) {
            ...
            if (targetElement.closest("[data-parent]")) {
                ...
                } else console.log("None sub-menu");
                e.preventDefault();
            }
        ...
        document.addEventListener("DOMContentLoaded", (() => {
            ...
            window.onscroll = function() {
                if (window.pageYOffset > 580) to_top_btn.style.display = "block"; else to_top_btn.style.display = "none";
            };
